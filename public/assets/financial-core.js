...
var incomePowerState = {
  ...
  function demoIsoDateDaysAgo(daysAgo) {
    var d = new Date();
    d.setHours(12, 0, 0, 0);
  ...
  function buildMockDemoDataSet() {
    var clA = {
      ...
      notes: 'Flagship retainer account.',
      totalRevenue: 0,
    ...
    var clB = {
      id: uuid(),
      companyName: 'Redwood Health',
      contactName: 'Jordan Patel',
      status: 'active',
      industry: 'Healthcare',
      email: 'finance@redwoodhealth.example',
      phone: '+1 (415) 555-0102',
      notes: 'Monthly growth support.',
      totalRevenue: 0,
    ...
    var clC = {
      ...
      status: 'active',
      industry: 'Fintech',
      email: 'ops@bluebirdfinance.example',
      phone: '+1 (415) 555-0103',
      notes: 'Project-based advisory.',
      totalRevenue: 0,
    ...
    var clD = {
      id: uuid(),
      companyName: 'Beacon Commerce',
      contactName: 'Taylor Brooks',
      status: 'active',
      industry: 'Ecommerce',
      email: 'billing@beaconcommerce.example',
    ...
    var clE = {
      id: uuid(),
      companyName: 'Summit Legal',
      contactName: 'Morgan Hayes',
      status: 'lead',
      industry: 'Professional Services',
      email: 'info@summitlegal.example',
      phone: '+1 (415) 555-0105',
      notes: 'Pipeline opportunity.',
      totalRevenue: 0,
      createdAt: Date.now() - 70 * 86400000,
      retainer: false,
    };

    var clientsSeed = [clA, clB, clC, clD, clE];
    ...
    var prA = {
      ...
      description: 'Lifecycle optimization and funnel improvements.',
      dueDate: demoIsoDateDaysAgo(-18),
      value: 38000,
      status: 'In progress',
    };
    var prB = {
      id: uuid(),
      clientId: clB.id,
      name: 'Retention Diagnostics',
      type: 'Analytics',
      description: 'Cohort and churn analysis implementation.',
      dueDate: demoIsoDateDaysAgo(6),
      value: 22000,
      status: 'In progress',
    };
    var prC = {
      id: uuid(),
      clientId: clC.id,
      name: 'Finance Onboarding Revamp',
      type: 'Product Marketing',
      description: 'Positioning refresh and launch assets.',
      dueDate: demoIsoDateDaysAgo(20),
      value: 27000,
      status: 'Not started',
    };
    var prD = {
      id: uuid(),
      clientId: clD.id,
      name: 'Summer Acquisition Campaign',
      type: 'Paid Media',
      description: 'Cross-channel demand generation.',
      dueDate: demoIsoDateDaysAgo(2),
      value: 31000,
      status: 'Complete',
    };
    var prE = {
      ...
      type: 'Reporting',
      description: 'Board-ready KPI reporting setup.',
      dueDate: demoIsoDateDaysAgo(38),
      value: 18000,
      status: 'Complete',
    };
    var projectsSeed = [prA, prB, prC, prD, prE];

    var tx = [];
    function addIncome(daysAgo, amount, category, source, clientId, projectId) {
      tx.push({
        id: uuid(),
        date: demoIsoDateDaysAgo(daysAgo),
        amount: amount,
        category: category,
        description: source,
        clientId: clientId || null,
    ...
    function addExpense(daysAgo, amount, category, title, vendor, clientId) {
      tx.push({
        id: uuid(),
        date: demoIsoDateDaysAgo(daysAgo),
        amount: amount,
        ...
        title: title,
        vendor: vendor,
        description: title + (vendor ? ' · ' + vendor : ''),
        clientId: clientId || null,
      });
    }

    addIncome(82, 14500, 'svc', 'Discovery + strategy workshop', clA.id, prA.id);
    addIncome(75, 9800, 'ret', 'Monthly retainer', clB.id, prB.id);
    addIncome(66, 17200, 'svc', 'Campaign implementation phase 1', clD.id, prD.id);
    addIncome(53, 10400, 'ret', 'Monthly retainer', clA.id, prA.id);
    addIncome(44, 13200, 'svc', 'Dashboard reporting sprint', clA.id, prE.id);
    addIncome(36, 8600, 'ret', 'Monthly retainer', clB.id, prB.id);
    addIncome(26, 15800, 'svc', 'Onboarding revamp kickoff', clC.id, prC.id);
    addIncome(15, 9100, 'ret', 'Monthly retainer', clA.id, prA.id);
    addIncome(7, 12600, 'svc', 'Acquisition campaign optimization', clD.id, prD.id);

    addExpense(83, 4200, 'sw', 'Creative Cloud licenses', 'Adobe', null);
    addExpense(79, 1800, 'sw', 'Cloud hosting', 'AWS', null);
    addExpense(73, 950, 'ads', 'Paid social test', 'Meta Ads', clD.id);
    addExpense(67, 2400, 'lab', 'Contract design support', 'Figma Talent', clA.id);
    addExpense(58, 1200, 'sw', 'CRM subscription', 'HubSpot', null);
    addExpense(49, 2600, 'ads', 'Search spend', 'Google Ads', clD.id);
    addExpense(41, 1350, 'oth', 'Client workshop travel', 'Delta', clA.id);
    addExpense(33, 2200, 'lab', 'Analytics contractor', 'Upwork', clB.id);
    addExpense(27, 780, 'sw', 'Automation tools', 'Zapier', null);
    addExpense(21, 3100, 'ads', 'Performance creative refresh', 'Meta Ads', clD.id);
    addExpense(14, 1650, 'sw', 'Data warehouse credits', 'Snowflake', null);
    addExpense(9, 900, 'oth', 'Team offsite prep', '', null);
    addExpense(5, 1450, 'lab', 'Freelance copywriting', 'Contra', clC.id);

    var invoiceSeed = [];
    var incomeTx = tx.filter(function (r) { return r.category === 'svc' || r.category === 'ret'; });
    if (incomeTx.length) {
      invoiceSeed.push({
        id: uuid(),
        incomeTxId: incomeTx[0].id,
        amount: incomeTx[0].amount,
        status: 'paid',
        dateIssued: demoIsoDateDaysAgo(80),
        dueDate: demoIsoDateDaysAgo(50),
        paidAt: demoIsoDateDaysAgo(56),
        number: 'INV-1001',
      });
    }
    if (incomeTx.length > 2) {
      invoiceSeed.push({
        id: uuid(),
        incomeTxId: incomeTx[2].id,
        amount: incomeTx[2].amount,
        status: 'sent',
        dateIssued: demoIsoDateDaysAgo(64),
        dueDate: demoIsoDateDaysAgo(34),
        number: 'INV-1002',
      });
    }
    if (incomeTx.length > 5) {
      invoiceSeed.push({
        id: uuid(),
        incomeTxId: incomeTx[5].id,
        amount: incomeTx[5].amount,
        status: 'draft',
        dateIssued: demoIsoDateDaysAgo(34),
        dueDate: demoIsoDateDaysAgo(4),
        number: 'INV-1003',
      });
    }
    ...
    var campaignsSeed = [
      {
        id: uuid(),
        date: demoIsoDateDaysAgo(42),
        channel: 'LinkedIn',
        campaignName: 'Fintech Pipeline Push',
        spend: 1800,
        leads: 26,
        conversions: 4,
        revenue: 12500,
        status: CAMPAIGN_STATUS_WON,
        pipelineValue: 0,
      },
      {
        id: uuid(),
        date: demoIsoDateDaysAgo(19),
        channel: 'Google Ads',
        campaignName: 'B2B Search Capture',
        spend: 2400,
        leads: 31,
        conversions: 3,
        revenue: 9800,
        status: CAMPAIGN_STATUS_PIPELINE,
        pipelineValue: 22000,
      },
      {
        id: uuid(),
        date: demoIsoDateDaysAgo(11),
        channel: 'Newsletter',
        campaignName: 'Executive Briefing Offer',
        spend: 420,
        leads: 14,
        conversions: 1,
        revenue: 0,
        status: CAMPAIGN_STATUS_LOST,
        pipelineValue: 0,
      },
    ];
  ...
  function loadMockDemoData() {
    var msg = 'Load mock demo data into this dashboard now?\n\nThis replaces current local clients, projects, transactions, invoices, campaigns, and timesheet entries.';
    if (!window.confirm(msg)) return;

    var data = buildMockDemoDataSet();
